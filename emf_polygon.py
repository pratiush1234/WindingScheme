...
def driver_code_1(theta):
    ...
    resistance = [round(sum(i)/len(theta),3) for i in res]
    # for i in magnitude:
...
#       combb.append(str(comb))
    ...
    return sorted(outputList, key = lambda x:x[2], reverse=True)[:5], resultant_dataframe.head(5), len(theta)
        ...
        if isinstance(element, tuple):
            ...
            for item in element:
                if isinstance(item, list):
                    ...
                    coils.append([1,key])
                    resistance.append(1/len(element))
                    break
            for ele in temp:
                temp_res += 1/ele
            if temp_res != 0:
...
def driver_code_2(theta):
    ...
    resultant_dataframe = outputDataframe.sort_values(by = 'EMF(pu)',ascending = False)
    resultant_dataframe.reset_index(drop = True,inplace = True)
    return sorted(out_coil, key = lambda x:x[2],reverse = True)[:5], resultant_dataframe.head(5), len(theta)
